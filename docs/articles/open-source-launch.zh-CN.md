...
下面的图片均来自真实钉钉会话。其他参与者的姓名、头像、工号和专家身份已经像素化；我的头像与 D仔头像保持一致；除隐私字段外，问题和回答正文没有改写。
...
### 案例二：问的是版本变化，回答必须回到当前仓库

我问 D仔：邮箱的已读、未读能力现在有没有，开源分支 `v1.0.47` 是不是才加上的？

![D仔核对邮箱已读未读能力](../assets/dzai-real-mail-qa.png)

D仔核对当前命令和文档后回答：`mail message search` 可以使用 `isRead:true/false` 过滤，文件夹和标签也有未读数字段；`v1.0.47` 主要是补齐并整理邮件命令树，并不是这项能力第一次出现。回答里同时给出了可继续核对的 [DWS 邮箱能力文档](https://github.com/DingTalk-Real-AI/dingtalk-workspace-cli/blob/main/skills/multi/dingtalk-mail/references/mail.md)。

这类问题不能只靠一份长期不更新的 FAQ。用户问的往往是“现在还有没有”“这个版本改了什么”，最终都要回到当前代码、当前命令说明和对应版本。

### 案例三：一句“没有报错”让听记问题进入第二轮排查

群里有同学问：以前能拿到听记逐字稿，现在为什么只有摘要，是不是功能被去掉了？

![D仔回答听记逐字稿问题](../assets/dzai-real-minutes-question.png)

第一轮里，D仔先确认获取逐字稿的命令仍然存在，再给出权限和分页两个排查方向，并请对方补充具体错误。

对方引用原消息追问：“没有报错，就说没有原文，只有摘要内容。”
...
### 案例四：跨组织群消息问题，机器人回答代码，专家回答策略

还有同学问：“群消息提取，有没有组织限制？”

![D仔区分 DWS 实现与平台权限策略](../assets/dzai-real-cross-org.png)

D仔能够从 DWS 代码层确认：`chat message list --group <id>` 没有再做一层群组织归属校验；但底层开放接口对跨企业群、外部群消息是否有额外权限限制，仅靠静态代码无法证明。它没有替平台策略拍板，而是把问题交给更合适的人。

后面的真实讨论串里，用户继续追问多个组织的情况，群内成员补充了“群在当前组织下”的预期边界，以及新版本对多组织登录的支持。
