...
| Coverage gate | `npm run test:coverage` on the release Node.js 24 line | 337/337 tests; coverage values recorded after the clean stacked-branch run below |
...
| Publisher-owned Runner kernel | Protocol, lease, replay, package snapshot and executor fixtures | Ed25519 task/receipt verification, canonical event hash chains, signed monotonic renewals, fencing identity, nonce replay rejection, exact local package digest, read-only one-run snapshot, lease abort and stable failure receipts verified |
...
| Compiled package | `npm run build`; `npm pack --dry-run --json` | `@fullstack-ai-infra/digital-employee@0.3.0` candidate built as 214 files/272,745 bytes, including the public core protocol and `host-runtime` Runner executor, lease, replay and package-digest exports |
